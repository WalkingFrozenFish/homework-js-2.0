...
function f6() {
    let keys = document.querySelector(".i-61").value;
    let values = document.querySelector(".i-62").value;

    a6[keys] = values;

    let res = ""
    for (const keys in a6) {
        res += `${keys} : ${a6[keys]} <br>`;
    }
...
document.querySelector('.b-10').onclick = () => {
    ...
    document.querySelector('.out-10').innerHTML = f10(a10, 5);
};
...
let a20 = {
    "red": [['Akademmistechko', 2], ['Nyvky', 2], ['Universytet', 3], ['Lisova', 2]],
    "blue": [['Minska', 1], ['Obolon', 0], ['Pochaina', 2], ['Holosiivska', 0]],
    "green": [['Syrets', 1], ['Zoloti Vorota', 2], ['Klovska', 0], ['Vidubichi', 2]],
}
