...
const f6 = () => {
    ...
    let setArr = new Set(a6);
    // console.log(setArr);
    // console.log(a6);
    out.innerHTML = setArr.size;
}
...
const f7 = () => {
    ...
    if(newInput.size == input.length && input.length > 6) {
        out.innerHTML = 1;
    } else {
        out.innerHTML = 0;
    }
...
const f11 = () => {
    ...
    s.add([1]);
    console.log(s);
    // Массивы не сравниваются по значению, а сравниваются по ссылке
}
...
const f13 = () => {
    // Преобразуем строку в массив с помощью перебора
    let arr = [];
    ...
    let obj = {}
    // Итерируем каждый уникальный элемент в множестве
    for (const iterator of arrSet) {
        let count = 1;
        // Перебираем каждую букву в массиве
        for (let i = 0; i < arr.length; i++) {
            // Если итерируемая буква равна итерируемой букве множества то
            if (iterator == arr[i]) {
                // Добавляем в пустой объект букву из массива и счетчик с единицей
                // Постфикс, будет возвращено старое значение!
                obj[arr[i]] = count++; 
            }
        }
        // Обнуляем счетчик для дальнейшего использования
        count = 1;
    }
    console.log(obj);
